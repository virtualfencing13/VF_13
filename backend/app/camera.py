                        ...
                        if self.ip_address:
                            try:
                                ...
                                if new_intrusion_state:
                                    ...
                                    state_cmd = "safe"
                                requests.get(f"http://{self.ip_address}/{state_cmd}", timeout=1)
                                print(f"📡 Wireless command [{state_cmd.upper()}] successfully sent to ESP32: {self.ip_address}")
                            except Exception as e:
                                print(f"⚠️ Failed to reach ESP32 safety node at {self.ip_address}: {e}")
